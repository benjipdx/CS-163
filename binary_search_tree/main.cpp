...
void display_all(job_manager & job_manager);
...
void welcome(){
	...
	cout << "      #                  #     #                                   "<<endl
		 ...
		 << "#     # #    # #    #    #     # #    # #   ##   #   #      #   #  "<<endl
		 <<  " #####   ####  #####     #     #  ####  #    #   #   ###### #    # "<<endl
		 << endl <<"Version 2.0, now with B I N A R Y  S E A R C H  T R E E functionality!" <<endl;
}
...
void add_job(job_manager & job_manager){
	...
	job_manager.store(new_job); //call the insert function passing in the new_job struct we just made
...
void display(job_manager & job_manager){
	...
	char * new_position_type = new char[strlen(temp_position_type)+1];
	strcpy(new_position_type, temp_position_type);
	
	//call function with needed info
	job_manager.display(new_position_type);
...
void remove(job_manager & job_manager){
	...
	job_manager.remove(new_position_type); //call the function with args
...
void find(job_manager & job_manager){
	...
	cout << "How many items would you like to store?" <<endl;
	cin >> array_size;
	cin.ignore(100, '\n');

	job * returned_jobs;
	returned_jobs = new job[array_size];

	job_manager.search(new_position_type, returned_jobs);

//the array now holds the data that we requested through the position type
//and this could then be passed to another function or output as found data
...
//function to display all the nodes in the bst, calls wrapper, that then calls real function
void display_all(job_manager & job_manager){

	job_manager.display_all();
